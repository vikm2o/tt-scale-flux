...
def prepare_latents(
        batch_size: int, height: int, 
        width: int, num_latent_channels: int, vae_scale_factor, 
        generator: torch.Generator, device: str, dtype: torch.dtype
    ) -> torch.Tensor:
...
def get_noises(seed: int, max_seed: int, num_samples: int, height: int, width: int, num_latent_channels: int, vae_scale_factor: int, device="cuda", dtype=torch.bfloat16) -> Dict[int, torch.Tensor]:
    seeds = torch.randint(0, high=max_seed, size=(num_samples, ), generator=torch.manual_seed(seed))
    return {
        int(noise_seed): prepare_latents(
            batch_size=1, height=height, width=width,num_latent_channels=num_latent_channels, vae_scale_factor=vae_scale_factor, generator=torch.manual_seed(int(noise_seed)), device=device, dtype=dtype
        ) 
...
def prompt_to_filename(prompt, max_length=100):
    ...
    if len(filename) > max_length:
        ...
        filename = f"{filename[:max_length - 9]}_{hash_digest}"
    
    return filename


def recover_json_from_output(output: str):
    stripped_out = output.replace("```", "").replace("json", "").strip()
    return json.loads(stripped_out)